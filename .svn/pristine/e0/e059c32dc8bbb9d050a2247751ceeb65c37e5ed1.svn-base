...
<form  role="form" method="post" action="<?php echo site_url('home/proses_tambah');?>">
<div class="col-lg-6">
<!--panel awal-->
...
<div class="panel-body">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="tanggal_lahir" name="tanggal_lahir" placeholder="____/__/__" value="<?php echo set_value('tanggal_lahir'); ?>>
  </div>
  <div class="form-group">
...
<?php
        echo form_dropdown("propinsi",$option_propinsi,"","id='id_propinsi'");

    ?>

	</select>  </div>
<div class="form-group">
    <label for="kabupaten">Kabupaten </label><br><div id="kab">Pilih Propinsi Dulu</div>


	</select>  </div>
<div class="form-group">
    <label for="kecamatan">Kecamatan </label><br><div id="kec">Pilih Kecamatan Dulu</div>
...
<script type="text/javascript">
        $("#id_propinsi").change(function(){
                var selectValues = $("#id_propinsi").val();
                if (selectValues == 0){
                    var msg = "kabupaten / Region :<br><select name=\"id_kabupaten\" disabled><option value=\"Pilih kabupaten / Kabupaten\">Pilih country Dahulu</option></select>";
                    $('#kab').html(msg);
                }else{
                    var id_propinsi = {id_propinsi:$("#id_propinsi").val()};
                    $('#id_kabupaten').attr("disabled",true);
                    $.ajax({
                            type: "POST",
                            url : "<?php echo site_url('home/kab')?>",
                            data: id_propinsi,
            ...
            $('body').delegate("#id_kabupaten","change", function() {
                var selectValues = $("#id_kabupaten").val();
                if (selectValues == 0){
                    var msg = "Pilih Propinsi Dahulu";
                    $('#kec').html(msg);
                }else{
                    var id_kabupaten = {id_kabupaten:$("#id_kabupaten").val()};
                    $('#id_kecamatan').attr("disabled",true);
                    $.ajax({
                            type: "POST",
                            url : "<?php echo site_url('home/kec')?>",
                            data: id_kabupaten,
                            success: function(msg){
                                $('#kec').html(msg);
                            }
